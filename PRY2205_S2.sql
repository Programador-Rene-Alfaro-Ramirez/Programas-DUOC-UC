-- -------------------------------------------------------- 
-- SEMANA 2 - PRY2205_SEMANA2 
-- Usuario: PRY2205_S2 
-- Base de datos: PRY2205_ACTIVIDAD_S2 (Oracle Cloud) 
--> Wallet: Wallet_T3MUGPU8W5I3FJR2 
-- Contraseña: PRY2205.semana_2
-- -------------------------------------------------------- 

--     Proyecto "Empresa de venta de calzados ShiverSale"
...
SELECT 
 ...
 TO_CHAR(f.fecha,'DD "de" Month YYYY')AS "Fecha emision",
 LPAD(f.rutcliente,10,'0')AS "RUT Cliente",
 TO_CHAR(f.neto,  '$999G999G999') AS "Monto Neto",
 TO_CHAR(f.iva,   '$999G999G999') AS "Monto Iva",
 TO_CHAR(f.total, '$999G999G999') AS "Total Factura",
 CASE
...
SELECT
  ...
  estado AS "ESTADO",
  CASE
    WHEN (saldo/credito)*100 > 80 THEN
      'Crítico'
    WHEN (saldo/credito)*100 BETWEEN 50 AND 80 THEN
...
SELECT
    ...
    NVL(TO_CHAR(p.valorcompradolar, 'FM999G999D00') || ' USD', 'Sin registro') AS "Compra en USD",
    
    CASE 
        WHEN p.valorcompradolar IS NULL THEN 'Sin registro'
        ELSE '$' || TO_CHAR(p.valorcompradolar * &TIPOCAMBIO_DOLAR, 'FM999G999G999') || ' PESOS'
    ...
    CASE
        WHEN p.totalstock > 80 THEN 
            '$' || TO_CHAR(p.vunitario * 0.9, 'FM999G999G999')
        ELSE 'N/A'
